        ...
        let pllsturl = this.pllstapi
            ...
            .then((data) => {
                for (var i = 0; i <= data.programs.length - 1; i++) {
                    let count = {}
                    window.alert(data.programs.length)
                    count.author = data.programs[i].dj.nickname
                        ...
                        .replace(':rid', rid)
                        .replace(':r', Math.random())
                        window.alert(songurl)
                        fetch(songurl)
                        .then((response1) => response1.json())
                        .then((data1) => {
                            count.url = data1.data[0].url
                            this.djplaylist.push(count)
                        })
                }
            })
			window.alert(this.djplaylist)
            this._loadPlayer(this.djplaylist)
    }
