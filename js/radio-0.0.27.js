class MetingRadioJSElement extends HTMLElement {
    ...
    _parse() {
        ...
        let pllsturl = this.pllstapi.replace(':type', this.meta.type).replace(':id', this.meta.id).replace(':r', Math.random())

        fetch(pllsturl).then((response)=>response.json()).then((data)=>{
            for (var i = 0; i <= data.programs.length - 1; i++) {
                ...
                let songurl = 'https://netease.project.ac.cn/song/url?id=:rid&r=:r'.replace(':rid', this.rid).replace(':r', Math.random())
                fetch(songurl).then((response1)=>response1.json()).then((data1)=>{
            ...
            Promise.all(djplaylist).then((kekka)=>window.alert(JSON.stringify(kekka))).then((kekka)=>this._loadPlayer(kekka))
        }
        )
    }
